(**** CSE341, Homework 4, CHALLENGE PROBLEMS: parsing JSON values ****)
...
(* Challenge Problem C1: (1+1) Write a consumer for string literals. String literals
...
(* Challenge Problem C2: (1+1) Write a consumer for the keywords true, false, and null.
...
(* Challenge Problem C3: (2+2) Complete the following definition of `tokenize_char_list`
...
(* Challenge Problem C4: (1+3) Write the top level tokenizer that takes a string,
...
(* Challenge Problem C5: (1+1) write a `parse_string` function that consumes a string
...
(* Challenge Problem C6: (2+2) write a function `expect` which consumes a single,
...
let rec parse_json (ts : token list) : json * token list =
    (* Challenge Problem C7: (2+2) write a `parse_field_value` function that parses one
  ...
  in
    (* Challenge Problem C8: (2+2) write a function `parse_field_value_list` that
  ...
  in
    (* Challenge Problem C9: (2+2) Write a function `parse_array_element_list` that
  ...
  in
  (* Challenge Problem C10: (2+2) complete the definition of `parse_json` by adding
...
(* Challenge Problem C11: (1+1) implement parser support for escape sequences
